...
const login_br13 = require('../model/login_br13'); // Importa o modelo Aluno
const bcrypt = require('bcrypt');
const router = express.Router();

// Exibe a página de login
router.get('/', (req, res) => {
    res.render('login_br13'); // Renderiza o formulário de login
});
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const login_white = await login_br13.findOne({ where: { nome } });
        ...
        res.redirect('/index_adm_br13'); // Mude para onde quiser redirecionar após login
    } catch (error) {
        console.error(error);
