...
router.get('/:nome', async (req, res) => {
  ...
  try {
    ...
    const equipamento = await equipamento_br13.findOne({ where: { nome: equipamentoNome } });
    ...
    await peca_br13.destroy({ where: { equipamento_nome: equipamentoNome } });

    // Deletar o equipamento
    await equipamento_br13.destroy({ where: { nome: equipamentoNome } });

    // Redirecionar após sucesso
    res.redirect('/tabelaEquipamento_br13');  // Redireciona para a tabela de equipamentos
